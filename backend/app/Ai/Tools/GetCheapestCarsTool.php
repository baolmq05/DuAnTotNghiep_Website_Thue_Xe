...
class GetCheapestCarsTool implements Tool
{
    /**
    ...
    {
        return 'Tìm kiếm danh sách các xe có mức giá thuê thấp nhất / rẻ nhất / tiết kiệm nhất trên nền tảng Drivio. Bắt buộc gọi tool này khi khách hàng hỏi về xe giá rẻ nhất, xe giá thấp nhất, xe bình dân, xe chi phí tiết kiệm nhất. Có thể lọc thêm theo từ khóa hoặc số chỗ ngồi nếu khách có yêu cầu cụ thể.';
    }
    ...
    public function handle(Request $request): Stringable|string
    {
        ...
        Log::info("AI Get Cheapest Cars - Keyword: '{$keyword}', Seat: '{$seatCount}', Limit: {$limit}");
        ...
        $cars = Car::query()
            ...
            ->orderByRaw('(unit_price - discount_value) ASC')
            ->limit($limit)
            ->get();
        ...
        if ($cars->isEmpty()) {
            return json_encode([
                'status' => 'empty',
                'message' => 'Hiện tại không tìm thấy xe giá rẻ nào phù hợp với yêu cầu của bạn trên hệ thống Drivio.',
                'cars' => []
        ...
        return json_encode([
            'status' => 'success',
            'message' => "Dưới đây là danh sách " . count($carList) . " xe có giá thuê thấp nhất (tiết kiệm nhất) trên Drivio:",
            'cars' => $carList
        ], JSON_UNESCAPED_UNICODE);
    }
    ...
    public function schema(JsonSchema $schema): array
    {
        return [
            'keyword' => $schema->string()
                ->description('Từ khóa lọc thêm tùy chọn (ví dụ: tên hãng xe "Toyota", loại xe "Sedan", "Hatchback", tính năng...). Có thể để trống "" nếu muốn tìm xe giá thấp nhất toàn hệ thống.')->nullable(),
            ...
            'limit' => $schema->integer()
                ->description('Số lượng xe giá thấp nhất cần lấy (mặc định là 5, tối đa là 10).')->nullable(),
        ];
    }
}
