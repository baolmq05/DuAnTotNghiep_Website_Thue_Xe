<template>
  <section class="max-w-7xl mx-auto px-4 py-12">
    ...
    <div class="relative">
      ...
      <!-- Next -->
      <button @click="scrollRight" :disabled="currentIndex >= promotions.length - itemsPerView" class="hidden md:flex absolute right-0 top-1/2 -translate-y-1/2 z-20
        w-10 h-10 rounded-full bg-white shadow-lg items-center justify-center
        disabled:opacity-30">
        ❯
      </button>
    ...
    <div class="flex justify-center mt-6 gap-2">
      ...
      <button v-for="index in promotions.length - itemsPerView + 1" :key="index" @click="currentIndex = index - 1"
        class="rounded-full transition-all" :class="currentIndex === index - 1
          ? 'bg-blue-600 w-6 h-2'
          : 'bg-gray-300 w-2 h-2'
          " />
...
p-4" @click.self="closeModal">

        <div class="bg-white rounded-[28px] overflow-hidden shadow-2xl max-h-[90vh] overflow-y-auto w-full
          max-w-xl no-scrollbar relative">

          <!-- Close -->
          <div class="flex justify-end p-4 absolute top-1 right-2 z-10">

            <button @click="closeModal" class="rounded-full bg-white flex items-center justify-center">
              <Icon name="ri:close-circle-fill" style="color:red;" class="w-7 h-7" />
            </button>

          </div>

          <!-- Banner -->
          <img :src="selectedPromotion?.image" class="w-full h-full object-cover relative top-0">
          <!-- Nội dung -->
          <div class="p-7">
            ...
            <!-- Copy -->
            <button @click="copyCode(selectedPromotion.code)" class="mt-8
              w-full
              bg-brand-primary
              hover:opacity-60
              text-white
...
const updateItemsPerView = () => {
  ...
  const max = Math.max(
    0,
    promotions.value.length - itemsPerView.value
  )
