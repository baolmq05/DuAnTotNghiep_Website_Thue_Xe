            ...
            >
                ...
                <div class="px-6 pt-6 pb-4 flex items-center justify-between border-b border-slate-100">
                    <h3 class="text-base font-bold text-slate-800 flex items-center gap-2">
                        <Icon name="lucide:credit-card" class="w-5 h-5 text-[#007aff]" />
                        {{ title }}
                    </h3>
                    <button 
                        @click="closeModal" 
                ...
                <div class="p-6 space-y-4">
                    ...
                    <div class="p-4 rounded-xl bg-slate-50 border border-slate-200/60 space-y-3">
                        ...
                        <div class="flex justify-between items-center text-sm font-semibold text-slate-700">
                            ...
                            <span class="text-base font-bold text-[#007aff]">{{ formatCurrency(amount) }}</span>
                        </div>
                    ...
                    <!-- Checkout guidelines -->
                    <div class="text-[11px] text-slate-500 space-y-1.5 bg-blue-50/50 border border-blue-100 rounded-lg p-3 leading-relaxed">
                        <p class="font-bold text-blue-800 flex items-center gap-1 text-xs">
                            <Icon name="lucide:info" class="w-4 h-4" />
                            Thông tin thanh toán thử nghiệm (ZaloPay Sandbox):
                        </p>
                        <ul class="list-disc list-inside pl-1 space-y-1 text-slate-600">
                            <li>Quét mã QR bằng ứng dụng ZaloPay Sandbox.</li>
                            <li>Hoặc chọn phương thức <strong>Thẻ ATM / Thẻ quốc tế</strong> trên trang cổng thanh toán và nhập thông tin thẻ giả lập bất kỳ để thử nghiệm.</li>
                        </ul>
                    </div>
                </div>
...
import ZaloPayButton from './ZaloPayButton.vue'
...
const props = withDefaults(defineProps<Props>(), {
    paymentType: 'rental',
    title: 'Xác nhận thanh toán qua ZaloPay',
    description: 'Hệ thống sẽ chuyển hướng bạn đến cổng thanh toán ZaloPay để xử lý giao dịch an toàn.'
})
